...
import style from './BTRenderDSPhim.module.css'


const mangPhim = [ // ở ngoài gọi là biến
    {
        "maPhim": 1321,
        "tenPhim": "Ted2",
        "biDanh": "ted2",
...
export default class BTRenderDSPhim extends Component { 

    //Bên trong khai báo thì gọi là thuộc tính và dùng this. để gọi thuộc tính đó
    ...
    renderPhim = () => {
        ...
        const jsxPhim = mangPhim.map((phim, index) => {
            return <div className="col-4 mb-4" key={index}>
                <div className="card">
                    {/* Các cách viết css   */}
                    {/* CÁCH 1 cho all style of project <img className={`card-img-top ${style.hinhAnhPhim}`} src={phim.hinhAnh} /> */}
                    {/* CÁCH 2 chỉ xài trên component <img className={`card-img-top ${style['hinhAnhPhim']}`}></img> */}
                    {/* Cách 3: thay đổi thuộc tính state thì dùng cách này */}
                    <img style={{
                        height: '200px',
                        width: 'auto',
                    }} className="card-img-top" src={phim.hinhAnh}></img>
                    <div className="card-body">
                        {/* Xét fix độ cao của tên phim  */}
                <h4 className="car-title" style={{height:'50px'}}>
                    {phim.tenPhim.length > 20 
                        ? <span>{phim.tenPhim.substr(0, 20)}...</span> 
                        : <span>{phim.tenPhim}</span>}</h4>
                        <p className="card-text">{phim.moTa.length > 40 
                            ? <span>{phim.moTa.substr(0,100)}...</span>
                            : <span>{phim.moTa}</span>
                        }</p>
                    </div>
                </div>
